...
public class MainActivity extends Activity {
    ...
    private View createOfficialHero() {
        ...
        TextView subtitle = micro("V12  •  CONJUGADO OBSIDIAN", CYAN);
    ...
    private View createSyncHomeCard() {
        ...
        LinearLayout top = horizontal();
        LinearLayout copy = vertical();
        copy.addView(micro("WITHINGS VIA HEALTH CONNECT", CYAN), full());
        TextView title = heading("Salvar atividade concluída", 19);
        title.setPadding(0, dp(3), 0, 0);
        copy.addView(title, full());
        top.addView(copy, weighted());

        String statusText = healthPermissionGranted ? "CONECTADO" : healthConnectAvailable ? "AUTORIZAR" : "VERIFICAR";
        ...
        TextView detail = body(healthPermissionGranted
                ? "Ao finalizar, o treino é registrado no Health Connect para o Withings importar como atividade."
                : "Conecte uma vez para registrar os treinos no Withings. A V12 não envia notificações ao relógio.", 13, MUTED);
        detail.setPadding(0, dp(10), 0, dp(12));
        card.addView(detail, full());

        Button setup = actionButton(healthPermissionGranted ? "REVISAR CONEXÃO" : "CONECTAR ATIVIDADE", false);
    ...
    private void finishWorkout(int workout) {
        ...
        if (!healthPermissionGranted) {
            Toast.makeText(
                    this,
                    "Treino concluído. Conecte o Health Connect para enviar a atividade ao Withings.",
                    Toast.LENGTH_LONG
            ).show();
            showHome();
            return;
        ...
        HealthConnectBridge.writeStrengthWorkout(this, title, notes, start, now, (success, message) -> {
            if (success) {
                ...
                Toast.makeText(
                        this,
                        "Treino salvo. O Withings poderá importar a atividade.",
                        Toast.LENGTH_LONG
                ).show();
            } else {
    ...
    private void showSyncSetup() {
        ...
        TextView eyebrow = micro("ATIVIDADE, NÃO NOTIFICAÇÃO", CYAN);
        eyebrow.setPadding(0, dp(8), 0, dp(5));
        root.addView(eyebrow, full());
        root.addView(heading("Conectar ao Withings", 31), full());
        ...
        TextView explanationText = body(
                "A V12 salva somente o treino concluído no Health Connect. O Withings importa esse registro como atividade. Nada é enviado como aviso ao relógio.",
                14,
                WHITE
        );
        ...
        root.addView(setupStep(
                "1",
                "Health Connect no celular",
                "É a ponte privada do Android usada para registrar o treino de força.",
                healthConnectAvailable ? "DISPONÍVEL" : "VERIFICAR",
                healthConnectAvailable ? GREEN : ORANGE
        ), fullWithBottom(8));
        ...
        root.addView(setupStep(
                "2",
                "Conferir os dados",
                "No Health Connect, a V12 terá apenas permissão para escrever exercícios. Ela não lê seus dados de saúde.",
                healthPermissionGranted ? "AUTORIZADO" : "PENDENTE",
                healthPermissionGranted ? GREEN : MUTED
        ), fullWithBottom(8));
        ...
        boolean withingsInstalled = isWithingsInstalled();
        root.addView(setupStep(
                "3",
                "Importar no Withings",
                "No Withings, abra Compartilhar, escolha Health Connect e permita a importação de Treinos.",
                withingsInstalled ? "ABRIR APP" : "INSTALAR APP",
                withingsInstalled ? CYAN : ORANGE
        ), fullWithBottom(8));
        Button withings = actionButton(withingsInstalled ? "ABRIR O WITHINGS" : "INSTALAR O WITHINGS", false);
        withings.setOnClickListener(view -> openWithings());
        root.addView(withings, fullWithBottom(12, 52));

        TextView footer = body(
                "Quando o último abdominal for concluído, a atividade “Treino da Luana A, B, C ou D” será gravada automaticamente. Sem notificações e sem comandos para o relógio.",
                12,
                MUTED
        );
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        boolean granted = HealthConnectBridge.permissionGrantedFromResult(resultCode, data);
        healthConnectAvailable = true;
        healthPermissionGranted = granted;
        preferences.edit()
                .putBoolean(HEALTH_AVAILABLE, true)
                .putBoolean(HEALTH_GRANTED, granted)
                .apply();
        Toast.makeText(
                this,
                granted ? "Atividades conectadas ao Health Connect" : "A permissão de treino não foi liberada",
                Toast.LENGTH_LONG
        ).show();
        showSyncSetup();
    }
    ...
    private void openWithings() {
        ...
        try {
            ...
            startActivity(new Intent(
                    Intent.ACTION_VIEW,
                    Uri.parse("https://play.google.com/store/apps/details?id=" + WITHINGS_PACKAGE)
            ));
        }
    }
    ...
    private void showPrivacyPolicy() {
        ...
        TextView title = heading("Como a V12 usa seus dados", 30);
        ...
        TextView text = body(
                "A V12 solicita apenas permissão para gravar no Health Connect uma sessão de treino de força concluída. O registro contém horário de início, horário de fim, letra do treino e grupo muscular.\n\n"
                        + "O aplicativo não lê peso, frequência cardíaca, sono, localização ou qualquer outro dado de saúde. Não há cadastro, servidor, anúncios ou envio para terceiros. As fotos e o progresso permanecem no celular.\n\n"
                        + "Você pode revogar a permissão a qualquer momento nas configurações do Health Connect. O compartilhamento posterior com o Withings depende exclusivamente das permissões escolhidas por você no próprio Withings.",
                14,
                WHITE
        );
